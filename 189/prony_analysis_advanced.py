import numpy as np
from scipy.linalg import svd, lstsq, toeplitz, eig
...
class EnhancedPronyAnalyzer:
    ...
    def tls_esprit(self, signal: np.ndarray, model_order: Optional[int] = None,
        ...
        U1_up = U1[:-1, :]
        U1_down = U1[1:, :]
        M_up = U1_up.conj().T @ U1_up
        M_down = U1_down.conj().T @ U1_down
        try:
            eigvals_up, eigvecs_up = eig(M_up)
            eigvals_down, eigvecs_down = eig(M_down)
            idx_up = np.argsort(np.abs(eigvals_up))[::-1]
            idx_down = np.argsort(np.abs(eigvals_down))[::-1]
            eigvecs_up = eigvecs_up[:, idx_up]
            eigvecs_down = eigvecs_down[:, idx_down]
            Phi = eigvecs_up @ np.linalg.inv(eigvecs_down)
        except:
            try:
                Phi, _, _, _ = lstsq(U1_up, U1_down, cond=1e-12)
            except:
                return self._empty_result(model_order, analytic)
        try:
            eigvals, _ = eig(Phi)
        except:
            return self._empty_result(model_order, analytic)
        return self._extract_modes(eigvals, analytic, model_order, s)

    def matrix_pencil_improved(self, signal: np.ndarray, pencil_param: Optional[int] = None,
        ...
        U1 = U[:, :model_order]
        V1 = Vh[:model_order, :].conj().T
        S1_inv = np.diag(1.0 / s[:model_order])
    ...
    def stabilization_diagram(self, signal: np.ndarray, method: str = 'tls_esprit',
                              order_range: range = range(4, 51, 2),
                              freq_tol: float = 0.02, damp_tol: float = 0.02) -> Dict:
        all_modes = []
        for order in order_range:
            try:
                if method == 'tls_esprit':
                    ...
                    result = self.matrix_pencil_improved(signal, model_order=order)
    ...
    def _cluster_modes(self, modes: List[Dict], freq_tol: float = 0.02,
                       damp_tol: float = 0.02, min_count: int = 5) -> List[Dict]:
    ...
    def analyze(self, signal: np.ndarray, method: str = 'tls_esprit',
        ...
        if use_stabilization:
            stab_kwargs = {
                ...
                'freq_tol': kwargs.pop('freq_tol', 0.02),
                'damp_tol': kwargs.pop('damp_tol', 0.02)
            }
            ...
            if method == 'tls_esprit':
                ...
                result = self.matrix_pencil_improved(processed, **kwargs)
    ...
    def _get_final_result(self, signal: np.ndarray, stab_result: Dict, method: str) -> Dict:
        ...
        if len(stable_modes) == 0:
            if method == 'tls_esprit':
                ...
                return self.matrix_pencil_improved(signal)
...
def plot_enhanced_results(result: Dict, save_path: str = None):
...
def main():
    ...
    noise_level = 0.02
    print("="*70)
    print("Testing Enhanced Prony Analysis with High Noise Level")
    ...
    print("\nTrue modes:")
    for i, mode in enumerate(true_modes):
        print(f"  Mode {i+1}: {mode['freq']:.2f} Hz, damping = {mode['damping']:.3f}")
    t, signal = generate_test_signal(fs=fs, duration=duration,
                                      modes=true_modes, noise_level=noise_level)
    analyzer = EnhancedPronyAnalyzer(fs=fs, freq_range=(0.1, 2.0))
    print("\n" + "="*70)
    print("1. Testing TLS-ESPRIT with Stabilization Diagram")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("2. Testing Improved Matrix Pencil with Stabilization Diagram")
    print("="*70)
    ...
    print("="*70)
    plot_enhanced_results(result_mp, save_path='matrix_pencil_result.png')
    print("\n" + "="*70)
    print("Performance Comparison (vs True Modes):")
    print("="*70)
    for result, method_name in [(result_tls, 'TLS-ESPRIT'), (result_mp, 'Matrix Pencil')]:
        print(f"\n{method_name}:")
