import numpy as np
from scipy.linalg import svd, lstsq, eig, sqrtm
...
class RobustPronyAnalyzer:
    ...
    def _cluster_modes(self, modes: List[Dict], freq_tol: float = 0.03,
                       damp_tol: float = 0.03, min_count: int = 4) -> List[Dict]:
...
def main():
    ...
    noise_level = 0.03
    print("="*70)
    print("Robust Prony Analysis - TLS-ESPRIT vs Matrix Pencil")
    print(f"Noise level: {noise_level} (higher noise for robustness test)")
    print("="*70)
    true_modes = [
        {'freq': 0.4, 'damping': 0.05, 'amp': 1.0, 'phase': 0.0},
    ...
    print("\n" + "="*70)
    print("1. Testing Matrix Pencil with Stabilization Diagram")
    print("="*70)
    result_mp = analyzer.analyze(signal, method='matrix_pencil', use_stabilization=True,
                                  svd_denoise=True, order_range=range(6, 31, 4))
    ...
    print("\n" + "="*70)
    print("2. Testing TLS-ESPRIT with Stabilization Diagram")
    print("="*70)
    ...
    print("="*70)
    print("\nPerformance Comparison (vs True Modes):")
    print("="*70)
    ...
    plot_results(result_mp, save_path='robust_matrix_pencil_result.png')
