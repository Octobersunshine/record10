...
class FMM2D:
    def __init__(self, p: int = 10, max_particles_per_leaf: int = 10, use_gpu: bool = True):
        self.p = p
    ...
    def _direct_interactions_gpu(self, all_particles_gpu: List[Particle]) -> None:
        n = len(all_particles_gpu)
        if n == 0:
            return

        x_gpu = self.xp.array([p.x for p in all_particles_gpu], dtype=self.xp.float64)
        y_gpu = self.xp.array([p.y for p in all_particles_gpu], dtype=self.xp.float64)
        charge_gpu = self.xp.array([p.charge for p in all_particles_gpu], dtype=self.xp.float64)
        ...
        for i, p in enumerate(all_particles_gpu):
    ...
    def _direct_interactions(self, node: QuadNode) -> None:
        ...
        if self.use_gpu and len(all_particles) > 100:
...
if __name__ == "__main__":
    ...
    fmm = FMM2D(p=10, max_particles_per_leaf=10, use_gpu=True)
    fmm_potentials = fmm.compute_potential(particles)
    fmm_time = time.time() - start_time
    print(f"FMM 耗时: {fmm_time:.4f} 秒")
    ...
    if CUPY_AVAILABLE:
        ...
        print(f"GPU 加速比: {direct_cpu_time / direct_gpu_time:.2f}x")

    error = np.mean(np.abs(np.array(fmm_potentials) - np.array(direct_potentials_cpu)))
    max_error = np.max(np.abs(np.array(fmm_potentials) - np.array(direct_potentials_cpu)))
    ...
    for i in range(min(10, n_particles)):
        print(f"粒子 {i:2d}: FMM = {fmm_potentials[i]:.6f}, 直接 = {direct_potentials_cpu[i]:.6f}, 差 = {abs(fmm_potentials[i] - direct_potentials_cpu[i]):.2e}")
